...
@Controller
@RequestMapping("/cityswitch")
public class CitySwitchController {
	
	
	@Autowired private CitySwitchService service;
	
	@RequestMapping(value = "/list/{city}/{type}/{page}", method = RequestMethod.GET, produces = { "application/json;charset=UTF-8" })
	@ResponseBody
	public ResultWithJson<ProductCollectDto> productCollectList(@PathVariable("city")String city,@PathVariable("type")String type, @PathVariable("page")int page){
		try {
			...
			System.out.println("搜索条件:城市="+city+",类别="+type);
