...
public class _bakFileUploadController {
	 
	 @RequestMapping(value = "/up", method = RequestMethod.POST,
	 		 produces = {"application/json;charset=UTF-8"})
	 public @ResponseBody ResultWithJson up(@RequestParam("imgurl") MultipartFile thumbnails,HttpServletRequest request){
		BufferedImage bi;
		try {
			if(thumbnails==null){
				return new ResultWithJson("1", "文件不能为空");
			}
			if(thumbnails.getSize()>4096000){
			...
			bi = ImageIO.read(thumbnails.getInputStream());
			...
			String fileName = currentTime + "." + suffix;// 当前时间+图片后缀名
			File tempFile = new File(filPath + fileName);
			System.out.println(tempFile);
			thumbnails.transferTo(tempFile);
			return new ResultWithJson("0", WxPayConfig.ACCESSUPLOADFILEPATH+currentTime+"."+suffix);
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 @RequestMapping(value={"/upload"}, method={org.springframework.web.bind.annotation.RequestMethod.POST}, produces={"application/json;charset=UTF-8"})
	  @ResponseBody
	  public ResultWithJson doUpload(@RequestParam("uploadFile") MultipartFile thumbnails, HttpSession session, HttpServletRequest request, HttpServletResponse response)
	  {
	  ...
	  @ResponseBody
	  public ResultWithJson doUploadPhp(HttpServletRequest request, HttpServletResponse response) throws IOException
	  {
		  ResultWithJson resultWithJson = null;
